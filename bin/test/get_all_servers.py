...
import os, sys, boto.ec2, datetime, time, calendar, ConfigParser
...
CIALL = os.path.join(path,'etc','apps','SplunkAppforAWS','log','ci11.txt')
RIALL = os.path.join(path,'etc','apps','SplunkAppforAWS','log','ri11.txt')
CICUR = os.path.join(path,'etc','apps','SplunkAppforAWS','log','ci1.txt')
RICUR = os.path.join(path,'etc','apps','SplunkAppforAWS','log','ri1.txt')
FINTMP = os.path.join(path,'etc','apps','SplunkAppforAWS','log','final22.txt')
RITMP = os.path.join(path,'etc','apps','SplunkAppforAWS','log','ri2.txt')
FINAL = os.path.join(path,'etc','apps','SplunkAppforAWS','log','instances.txt')
...
for key,dept in parser.items("keys"):
	...
	if len(cols) > 2:
		...
		rline = rline.rstrip()
		try:
			ec2_conn = boto.ec2.connect_to_region(str(rline),aws_access_key_id=cols[1],aws_secret_access_key=cols[2])
		except boto.exception.EC2ResponseError, emsg:
			fERRFL.write(now+' '+str(key)+' '+dept+' '+rline+'\n')
			continue
		running_all = ec2_conn.get_all_instances()
		for running_instance in running_all:
			for instance in running_instance.instances:
#				pprint (instance.__dict__)
		...
		for reserved_instance in ec2_conn.get_all_reserved_instances():
			if reserved_instance.state == "active":
				rn = reserved_instance.availability_zone
				ritype = reserved_instance.instance_type
                               	fRICUR.write(now+' instancetype='+ritype+' region='+rn+' subaccount='+cols[0]+' instancecount='+str(reserved_instance.instance_count)+'\n')
